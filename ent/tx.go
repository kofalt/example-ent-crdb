...
func (tx *Tx) init() {
	...
	tx.UserPromoCode = NewUserPromoCodeClient(tx.config)
	tx.Vehicle = NewVehicleClient(tx.config)
	tx.VehicleLocationHistory = NewVehicleLocationHistoryClient(tx.config)
}
...
// applies a query, for example: Ride.QueryXXX(), the query will be executed
