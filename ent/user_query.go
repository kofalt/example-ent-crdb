...
type UserQuery struct {
	...
	withRides          *RideQuery
	withUserPromoCodes *UserPromoCodeQuery
...
func (uq *UserQuery) QueryVehicles() *VehicleQuery {
	query := (&VehicleClient{config: uq.config}).Query()
...
func (uq *UserQuery) QueryRides() *RideQuery {
	query := (&RideClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryUserPromoCodes() *UserPromoCodeQuery {
	query := (&UserPromoCodeClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withVehicles:       uq.withVehicles.Clone(),
		withRides:          uq.withRides.Clone(),
		withUserPromoCodes: uq.withUserPromoCodes.Clone(),
...
// WithUserPromoCodes tells the query-builder to eager-load the nodes that are connected to
// the "user_promo_codes" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithUserPromoCodes(opts ...func(*UserPromoCodeQuery)) *UserQuery {
	query := (&UserPromoCodeClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserPromoCodes = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withVehicles; query != nil {
		if err := uq.loadVehicles(ctx, query, nodes,
			func(n *User) { n.Edges.Vehicles = []*Vehicle{} },
			func(n *User, e *Vehicle) { n.Edges.Vehicles = append(n.Edges.Vehicles, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withRides; query != nil {
		if err := uq.loadRides(ctx, query, nodes,
			func(n *User) { n.Edges.Rides = []*Ride{} },
			func(n *User, e *Ride) { n.Edges.Rides = append(n.Edges.Rides, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withUserPromoCodes; query != nil {
		if err := uq.loadUserPromoCodes(ctx, query, nodes,
			func(n *User) { n.Edges.UserPromoCodes = []*UserPromoCode{} },
			func(n *User, e *UserPromoCode) { n.Edges.UserPromoCodes = append(n.Edges.UserPromoCodes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadVehicles(ctx context.Context, query *VehicleQuery, nodes []*User, init func(*User), assign func(*User, *Vehicle)) error {
	...
	query.Where(predicate.Vehicle(func(s *sql.Selector) {
		s.Where(sql.InValues(user.VehiclesColumn, fks...))
	}))
...
func (uq *UserQuery) loadRides(ctx context.Context, query *RideQuery, nodes []*User, init func(*User), assign func(*User, *Ride)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Ride(func(s *sql.Selector) {
		s.Where(sql.InValues(user.RidesColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (uq *UserQuery) loadUserPromoCodes(ctx context.Context, query *UserPromoCodeQuery, nodes []*User, init func(*User), assign func(*User, *UserPromoCode)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.UserPromoCode(func(s *sql.Selector) {
		s.Where(sql.InValues(user.UserPromoCodesColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
